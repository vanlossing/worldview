...
export function timelineConfig(models, config, ui) {
    ...
    switch (level) {
      case 1: // Year
        ...
        paddedRange = [
          ...
          new Date(
            tl.data.end().setUTCFullYear(tl.data.end().getUTCFullYear() + 10)
          )
        ];

        altEnd = new Date(
          tl.data.start().getUTCFullYear() + tickCountMax,
        ...
        tl.zoom.current.ticks.boundary.first = function() {
          ...
          return new Date(
            Date.UTC(Math.floor(first.getUTCFullYear() / 10) * 10, 0, 1)
          );
        };
        ...
        tl.zoom.current.ticks.boundary.last = function() {
          ...
          return new Date(
            Date.UTC(Math.ceil(last.getUTCFullYear() / 10) * 10, 0, 1)
          );
        };
      ...
      case 2: // Month
        ...
        paddedRange = [
          new Date(
            tl.data.start().setUTCFullYear(tl.data.start().getUTCFullYear() - 1)
          ),
          new Date(
            tl.data.end().setUTCFullYear(tl.data.end().getUTCFullYear() + 1)
          )
        ];
        ...
        tl.zoom.current.ticks.boundary.first = function() {
          var first = tl.ticks.normal.firstDate;
          return new Date(Date.UTC(first.getUTCFullYear(), 0, 1));
        };
        ...
        tl.zoom.current.ticks.boundary.last = function() {
          var last = tl.ticks.normal.lastDate;
          return new Date(Date.UTC(last.getUTCFullYear() + 1, 0, 1));
        };
      ...
      case 3: // Day
        ...
        paddedRange = [
          new Date(
            tl.data.start().setUTCDate(tl.data.start().getUTCDate() - 15)
          ),
  ...
  var switchZoom = function(zoomLevel) {
    ...
    clone = $(zoomElement)
      .clone()
      .prop('id', 'current-zoom')
      .removeClass('zoom-btn-inactive')
      .addClass('zoom-btn-active');
